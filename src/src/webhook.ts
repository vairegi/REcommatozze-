...
// ---------------------------------------------------------------------------
// /channels — native TEXT spreadsheet (v2; the v1 image board was rolled back).
// Monospace <pre> table, channel names carry the invite link inline
// (<a href>), and exactly ONE 🔄 Refresh inline button on the last chunk.
// Data is pulled from Turso + live-verified on EVERY call.
// ---------------------------------------------------------------------------

// Tracks each admin's board message ids so Refresh can delete + resend.
// Memory-only: after a Render restart the first Refresh just posts a fresh board.
const channelBoardMessages = new Map<number, number[]>();

const CHB_PARTNER_BOT = "@InsideAds_bot";
const CHB_PRIORITY = ["MINE", "ADULT", "MANGA"];
const CHB_MAXLEN = 3400; // safe under the 4096 cap incl. <pre> tags + header
...
async function gatherChannelEntries(deps: {
  ...
  // MINE → ADULT → MANGA → other lists → [NONE] last; stable within a tier.
...
/** One spreadsheet row, two narrow monospace lines so nothing wraps on mobile. */
function chbRow(num: number, e: ChbEntry): string {
  const n = String(num).padStart(2, " ");
  const name = escapeHtml(e.title.length > 30 ? e.title.slice(0, 29) + "…" : e.title);
  const linked = e.url ? `<a href="${escapeHtml(e.url)}">${name}</a>` : name;
  const tags = e.cats.length ? e.cats.join("|") : "NONE";
  const flags = `${e.iads ? " ✅IAds" : ""}${e.locked ? " 🔒" : ""}`;
  return `${n}. ${linked}\n    <code>${e.chatId}</code> · ${escapeHtml(tags)}${flags}`;
}

/** Build chunked <pre> messages. The Refresh button goes on the LAST chunk only. */
function buildChannelBoardChunks(entries: ChbEntry[]): string[] {
  ...
  const sections: { head: string; rows: ChbEntry[] }[] = [];
  if (buckets.channel.length) sections.push({ head: `📢 CHANNELS (${buckets.channel.length})`, rows: buckets.channel });
  if (buckets.supergroup.length) sections.push({ head: `👥 SUPERGROUPS (${buckets.supergroup.length})`, rows: buckets.supergroup });
  if (buckets.group.length) sections.push({ head: `👥 GROUPS (${buckets.group.length})`, rows: buckets.group });

  let num = 0;
  const blocks: string[] = [];
  for (const s of sections) {
    blocks.push(`\n${s.head}\n${"─".repeat(28)}`);
    for (const e of s.rows) blocks.push(chbRow(++num, e));
  }
  ...
  const header = `📋 <b>Channels spreadsheet</b> — ${entries.length} chats · ${escapeHtml(stamp)} IST\n`;

  const chunks: string[] = [];
  let cur = "";
  for (const b of blocks) {
    const candidate = cur ? cur + "\n" + b : b;
    if (cur && (header + "<pre>" + candidate + "</pre>").length > CHB_MAXLEN) {
      chunks.push(cur);
      cur = b;
    } else {
      cur = candidate;
  ...
  return chunks.map((body, i) => {
    ...
    return (i === 0 ? header : "") + `<pre>${body.trim()}</pre>` + (part ? `\n<i>page${part}</i>` : "");
  });
}

async function sendChannelBoard(dmChatId: number, entries: ChbEntry[], telegramCall: any): Promise<void> {
  // Remove the previous board so Refresh never leaves stale duplicates.
...
async function handleChannelListRefresh(cq: any) {
  ...
  await sendChannelBoard(chatId, entries, telegramCall); // deletes old chunks, posts fresh
}

// ---- legacy implementation kept for reference (unused) ----
async function handleChannelsCommandLegacy(args: {
  dmChatId: number;
  supabaseAdmin: any;
  telegramCall: (m: string, b?: Record<string, unknown>) => Promise<any>;
  getBotIdentity: () => Promise<{ id: number; username?: string }>;
